...
if __name__ == '__main__':
    ...
    测试补全的功能是否正确
    """
    kg_data = load_kg("../data/Douban/kg_final.csv")
    n_relations = 0  # 计算关系数
    n_entities = 0  # 计算实体数
    data = {}
    data['h'] = []
    ...
    for i in range(0, kg_data.shape[0]):
        ...
        if (kg_data.loc[i]["h"] > n_entities):
            n_entities = kg_data.loc[i]["h"]
        if (kg_data.loc[i]["t"] > n_entities):
            n_entities = kg_data.loc[i]["t"]
    n_relations += 1
    n_entities += 1
