...
def load_data(data_path):
    ...
    if os.path.exists(cache_path):  #判断是否存在缓存文件
        ...
        print("加载新数据中...")
        #设置要加载的数据字段的类型
        dtype={"User": np.int32, "Book": np.int32, "Rate": np.float32}
        #加载数据，我们只用前三列数据，分别是用户ID、书籍ID、以及用户对书籍的对应评分
        ratings=pd.read_csv(DATA_PATH, dtype=dtype, usecols=range(3))
        #设置Embedding掩盖数组
        for row in range(0,len(ratings)):
            if Embedding[row] == 1:
                ratings.at[row, "Rate"] = None
        #透视表，将书籍ID转换为列名称，转换成为一个User-Book的评分矩阵
        ratings_matrix = pd.pivot_table(data=ratings, index=["User"], columns=["Book"], values="Rate")
        #存入缓存文件
...
def predict_userBasedCF(uid, iid, ratings_matrix, user_similar):
    ...
    for sim_uid in finally_similar_users.index:
        ...
        #近邻用户的评分数据
        sim_user_rated_books=ratings_matrix.loc[sim_uid].dropna()
        #近邻用户对iid物品的评分
        sim_user_rating_for_item=sim_user_rated_books[iid]
        #计算分子的值
    ...
    #print("预测出用户{0}对书籍{1}的评分为：{2:.2f}".format(uid, iid, predict_rating))
...
#透视表，将书籍ID转换为列名称，转换成为一个User-Book的评分矩阵
ratings_matrix_2 = pd.pivot_table(data=ratings_2, index=["User"], columns=["Book"], values="Rate")
...
for row in range(0, len(ratings_2)):
    if Embedding[row] == 1:       #为测试数据
        #预测的分数
        predict_rating = get_predict(ratings_2.at[row, "User"], ratings_2.at[row, "Book"], ratings_matrix, user_similar)
        #瞎猜的分数
