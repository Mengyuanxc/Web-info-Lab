...
class DataLoader(DataLoaderBase):
    ...
    def construct_data(self, kg_data):
        ...
        inverse_kg_data['h'], inverse_kg_data['t'] = inverse_kg_data['t'], inverse_kg_data['h']
        inverse_kg_data['r'] += len(kg_data['r'].unique())
        ...
        self.n_relations = len(kg_data['r'].unique())
        ...
        self.cf_train_data = (self.cf_train_data[0] + self.n_entities, self.cf_train_data[1] + self.n_entities)
        self.cf_test_data = (self.cf_test_data[0] + self.n_entities, self.cf_test_data[1] + self.n_entities)
        ...
        cf2kg_train_data['h'] = self.cf_train_data[0].flatten()
        cf2kg_train_data['t'] = self.cf_train_data[1].flatten()
        ...
        inverse_cf2kg_train_data['h'] = self.cf_train_data[1].flatten()
        inverse_cf2kg_train_data['t'] = self.cf_train_data[0].flatten()
        ...
        for i in range(self.n_kg_train):
            h, r, t = self.kg_train_data.loc[i]
            if h not in self.train_kg_dict:
                self.train_kg_dict[h] = []
            self.train_kg_dict[h].append((t, r))

            if r not in self.train_relation_dict:
                self.train_relation_dict[r] = []
    ...
    def create_laplacian_dict(self):
        ...
        for r, adj in self.adjacency_dict.items():
            self.laplacian_dict[r] = sp.coo_matrix(norm_lap_func(adj))
