...
rating = pd.read_csv(data_path, dtype={
    "User": np.int32, "Movie": np.int32, "Rate": np.float32}, usecols=range(3))
rating_matrix_2 = pd.pivot_table(data=rating, index=["User"], columns=["Movie"], values="Rate")
...
for row in range(0, len(rating)):
    if embedding[row] == 1:
        predict_rating = functions.predict_score(rating.at[row, "User"], rating.at[row, "Movie"], rating_matrix, user_similarity)
